...
SELECT 
    ...
    -- Convert sales order date only if it’s a valid 8-digit number
    CASE 
...
SELECT
    CASE
        WHEN cid LIKE 'NAS%' THEN SUBSTRING(cid FROM 4)  -- Remove 'NAS' prefix (starting from 4th char)
        ELSE cid
    END AS cid,

    CASE
        WHEN bdate > CURRENT_DATE THEN NULL  -- Replace GETDATE() with CURRENT_DATE
        ELSE bdate
    END AS bdate,
